package jdbc.task2.dao.impl;

import jdbc.task2.dao.MovieDAO;
...
/* Sens data to database */

public class MovieDAOImpl implements MovieDAO {
    private final String TABLE_NAME;
    private final Connection conn;

    public MovieDAOImpl(String TABLE_NAME, Connection conn) {
    ...
    public void insert(MovieEntity movieEntity) {
        String insertQuery = String.format("""
                ...
                VALUES
                ('%s', '%s', %s)
                """, TABLE_NAME, movieEntity.getTitle(), movieEntity.getGenre(), movieEntity.getYearOfRelease());
    ...
    public Optional<MovieEntity> findById(int id) {
        ...
        try (PreparedStatement findByIdStmt = conn.prepareStatement(findByIdQuery)) {
            ...
            if (resultSet.next()) {
                tempMovieEntity = mapResultSetToMovieEntity(resultSet);
            }
        ...
        return verifyMappingReturn(tempMovieEntity);
    }

    private Optional<MovieEntity> verifyMappingReturn(MovieEntity tempMovieEntity) {
        return tempMovieEntity.getId() != -1 ? Optional.of(tempMovieEntity) : Optional.empty();
    }

    private MovieEntity mapResultSetToMovieEntity(ResultSet resultSet) throws SQLException {
        MovieEntity tempMovieEntity = new MovieEntity();
    ...
    public List<MovieEntity> findMoviesWhereGenreIs(String genre) {
        ...
        try (PreparedStatement findMoviesWithGenreOfStmt =
            ...
            while (resultSet.next()) {
                result.add(mapResultSetToMovieEntity(resultSet));
            }
